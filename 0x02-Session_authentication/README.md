...
* If `auth.authorization_header(request)` is `None` and `auth.session_cookie(request)` is `None` return `None`, `abort(401)`
...
[8. Logout](./api/v1/auth/session_auth.py), [views](./api/v1/views/session_auth.py)
...
[9. Expiration?](./api/v1/auth/session_exp_auth.py), [app](./api/v1/app.py)
...
[10. Sessions in database](./api/v1/auth/session_db_auth.py), [app](./api/v1/app.py), [user_session](./models/user_session.py)
